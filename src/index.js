...
client.on('interactionCreate', async i => {
 try {
  ...
  if (
   ...
   const username = i.fields
    .getTextInputValue('roblox_username')
    .trim();

   if (!username) {
    return i.reply({
     content: '请输入 Roblox 用户名。',
     ephemeral: true
    });
   }
   ...
   if (existing) {
    return i.reply({
     content:
      `这个 Roblox 账号已经在白名单中：${existing.roblox_name}。`,
     ephemeral: true
    });
   }
   ...
   if (binding) {
    return i.reply({
     content:
      `这个 Roblox 账号已经被 Discord 用户 ${binding.discord_name} 绑定。`,
     ephemeral: true
    });
   }
   ...
   log(
    ...
    '管理员直接添加 Roblox 白名单，无需 Discord /bind'
   );
