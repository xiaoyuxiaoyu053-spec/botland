...
client.on('interactionCreate', async i => {
  try {
    ...
    if (
      !isManager(i) &&
      ['players', 'add', 'logs', 'blacklist', 'admins'].includes(i.customId)
    ) {
      return i.reply({
        content: '没有白名单管理权限。',
        ephemeral: true
    ...
    if (i.isStringSelectMenu() && i.customId === 'pickplayer') {
      const x = db.prepare(
    ...
    if (
      i.isButton() &&
      (i.customId.startsWith('remove:') || i.customId.startsWith('ban:'))
    ) {
    ...
    if (i.isButton() && i.customId === 'add') {
      return i.reply({
        content:
          '添加白名单建议使用 `/bind`。如果要由管理员代绑任意 Roblox 账号，可继续扩展为 Modal。',
        ephemeral: true
      });
    }
    ...
    if (
      i.isButton() &&
      (i.customId.startsWith('grant:') || i.customId.startsWith('revoke:'))
    ) {
      if (!isOwner(i)) {
        return i.reply({
          content: '只有服务器 Owner 可以管理管理员。',
