  ...
  try {
    if (i.isChatInputCommand()) {
      ...
      if (i.commandName === 'bind') {
        ...
        if (db.prepare(
          ...
          return i.reply({
            content: '🚫 你的 Discord 账号已永久禁止绑定任何 Roblox 账号。',
            ephemeral: true
          });
        }
        ...
        if (db.prepare(
          ...
          return i.reply({
            content: '🚫 这个 Roblox 账号已被永久封禁，无法绑定。',
            ephemeral: true
          });
        }
        ...
        if (existsD) {
          return i.reply({
            content: `你已经绑定 Roblox：${existsD.roblox_name}。请先 /unbind。`,
            ephemeral: true
          });
        }
        ...
        if (existsR) {
          return i.reply({
            content: '这个 Roblox 账号已经被其他 Discord 账号绑定。',
            ephemeral: true
          });
        }
        ...
        db.transaction(() => {
          ...
          db.prepare(`
            INSERT OR REPLACE INTO whitelist(
              roblox_id, roblox_name, discord_id,
              discord_name, source, created_at
            )
            VALUES(?,?,?,?,?,?)
          `).run(
            String(u.id),
      ...
      if (i.commandName === 'unbind') {
        ...
        await i.reply({
          content: `✅ 已解绑 Roblox：${b.roblox_name}`,
          ephemeral: true
        });
    ...
    if (i.isButton() && i.customId === 'add') {
      ...
      const modal = new ModalBuilder()
        .setCustomId('admin_add_whitelist')
        .setTitle('添加 Roblox 白名单');
    ...
    if (i.isModalSubmit() && i.customId === 'admin_add_whitelist') {
      if (!isManager(i)) {
        return i.reply({
          content: '没有白名单管理权限。',
          ephemeral: true
      ...
      const roblox = await robloxUser(robloxName).catch(() => null);
      ...
      log(
        ...
        `管理员 ${i.user.username} 直接添加 Roblox 白名单`
      );
    ...
    if (i.isButton() && i.customId === 'logs') {
      const a = db.prepare(`
        ...
        WHERE guild_id=?
        AND action IN ('普通删除','永久封禁','解绑','添加白名单')
        ORDER BY id DESC
        LIMIT 20
      `).all(i.guildId);
    ...
    if (i.isUserSelectMenu() && i.customId === 'adminuser') {
      if (!isOwner(i)) {
        return i.reply({
          content: '只有服务器 Owner 可以管理管理员。',
          ephemeral: true
    ...
    if (!i.replied && !i.deferred) {
      await i.reply({
        content: '操作失败，请检查 Bot 权限或 Render 日志。',
        ephemeral: true
      }).catch(() => {});
    }
  }
